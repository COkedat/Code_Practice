...
두부게임은 N명이서 원모양으로 앉아서 하는 게임이다. 주최자(A)를 기준으로 M 모 게임(M은 1보다 큰 홀수 이다)을 진행한다고 하자. 주최자(A)는 (M/2+1)의 정수부분으로 두부 모 수의 기준점이 되고 주최자 뒷 번호는 차례대로 정수부분 M/2+2 ~ M 모가 된다. 같은 원리로 주최자 앞 번호는 정수부분 M/2 ~ 1모가 되는 것이다.
...
inp = list(map(int, input().split(" "))) # N, M, A
N = inp[0] # 참여자수
M = inp[1] # M모 게임
A = inp[2] # 시작점 (주최자)
for i in range(10000):
    tmp = int(stdin.readline()) #두부 모 수
    if(int(M/2+1)==tmp): # 자신의 위치를 불렀을 경우
        print(0)
        break # 종료
    elif(int(M/2+1)<tmp): # 자신보다 높은 모를 불렀을 경우
        A = A + (tmp - int(M/2+1))
        if(A > N): # 만약 참여자 수를 넘어갔다면 루프
            A = A - N
        print(A)
    else: # 자신보다 낮은 모를 불렀을 경우
        A = A - (int(M/2+1)-tmp)
        if(A < 1): # 만약 참여자 수를 넘어갔다면 루프
            A = A + N
        print(A)
